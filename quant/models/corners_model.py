...
class CornersModel:
    ...
    def fit(self, completed_matches: list[dict]) -> None:
        ...
        for m in valid:
            ...
            for team in (home, away):
                if team not in raw:
                    raw[team] = {
                        "home_won": 0.0, "home_conceded": 0.0, "home_n": 0,
                        "away_won": 0.0, "away_conceded": 0.0, "away_n": 0,
                    }
        ...
        for team, v in raw.items():
            ...
            fitted[team] = {
                "attack_home":  avg_hw / self.league_avg_home_corners if self.league_avg_home_corners else 1.0,
                "defense_home": avg_hc / self.league_avg_away_corners if self.league_avg_away_corners else 1.0,
                "attack_away":  avg_aw / self.league_avg_away_corners if self.league_avg_away_corners else 1.0,
                "defense_away": avg_ac / self.league_avg_home_corners if self.league_avg_home_corners else 1.0,
            }
    ...
    def expected_corners(self, home_team: str, away_team: str) -> tuple[float, float]:
        home_p = self._team_stats.get(home_team, {"attack_home": 1.0, "defense_home": 1.0, "attack_away": 1.0, "defense_away": 1.0})
        away_p = self._team_stats.get(away_team, {"attack_home": 1.0, "defense_home": 1.0, "attack_away": 1.0, "defense_away": 1.0})

        lam_home = max(0.5, self.league_avg_home_corners * home_p["attack_home"] * away_p["defense_away"])
        lam_away = max(0.5, self.league_avg_away_corners * away_p["attack_away"] * home_p["defense_home"])
